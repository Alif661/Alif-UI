--[[
	...
	Alif Ananta Interface Suite
	by Alif Ananta

Alif Ananta  | Designing + Programming
Alif Ananta   | Programming
Alif Ananta    | Programming
Alif Ananta | Programming
...
local requestsDisabled = true --getgenv and getgenv().DISABLE_ALIFNETWORK_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local AlifNetworkFolder = "AlifNetwork"
local ConfigurationFolder = AlifNetworkFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		alifnetworkOpen = {Type = 'bind', Value = 'K', Name = 'AlifNetwork Keybind'},
		-- buildwarnings
		-- alifnetworkprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.alifnetworkOpen"] = "J"
...
local prompt = useStudio and require(script.Parent.prompt) or loadWithTimeout('https://raw.githubusercontent.com/Alif661/Alif-UI/refs/heads/master/prompt.lua')
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(AlifNetworkFolder) then
				if isfile and isfile(AlifNetworkFolder..'/settings'..ConfigurationExtension) then
					file = readfile(AlifNetworkFolder..'/settings'..ConfigurationExtension)
				end
			end
	...
	if not success then 
		if writefile then
			warn('AlifNetwork had an issue accessing configuration saving capability.')
		end
	end
end
...
if not requestsDisabled then
	...
	if cachedSettings and (#cachedSettings == 0 or (cachedSettings.System and cachedSettings.System.usageAnalytics and cachedSettings.System.usageAnalytics.Value)) then
		sendReport("execution", "AlifNetwork")
...
local AlifNetwork = useStudio and script.Parent:FindFirstChild('AlifNetwork') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local alifnetworkDestroyed = false -- True when AlifNetworkLibrary:Destroy() is called

repeat
	if AlifNetwork:FindFirstChild('Build') and AlifNetwork.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('AlifNetwork | Build Mismatch')
		print('AlifNetwork may encounter issues as you are running an incompatible interface version ('.. ((AlifNetwork:FindFirstChild('Build') and AlifNetwork.Build.Value) or 'No Build') ..').\n\nThis version of AlifNetwork is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, AlifNetwork = AlifNetwork, useStudio and script.Parent:FindFirstChild('AlifNetwork') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

AlifNetwork.Enabled = false

if gethui then
	AlifNetwork.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(AlifNetwork)
	AlifNetwork.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	AlifNetwork.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == AlifNetwork.Name and Interface ~= AlifNetwork then
			Interface.Enabled = false
			Interface.Name = "AlifNetwork-Old"
		end
	end
elseif not useStudio then
...
if AlifNetwork.AbsoluteSize.X < minSize.X and AlifNetwork.AbsoluteSize.Y < minSize.Y then
...
local Main = AlifNetwork.Main
local MPrompt = AlifNetwork:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = AlifNetwork:FindFirstChild('Drag')
...
AlifNetwork.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/Alif661/Alif-UI/refs/heads/master/icons.lua')
-- Variables
...
local Notifications = AlifNetwork.Notifications

local SelectedTheme = AlifNetworkLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = AlifNetworkLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	AlifNetwork.Main.BackgroundColor3 = SelectedTheme.Background
	AlifNetwork.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	AlifNetwork.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	AlifNetwork.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	AlifNetwork.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	AlifNetwork.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	AlifNetwork.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		AlifNetwork.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		AlifNetwork.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(AlifNetwork:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("AlifNetwork | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("AlifNetwork | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('AlifNetwork had an issue decoding the configuration file, please try delete the file and reopen AlifNetwork.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(AlifNetworkLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("AlifNetwork | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--AlifNetworkLibrary:Notify({Title = "AlifNetwork Flags", Content = "AlifNetwork was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(AlifNetworkLibrary.Flags) do
...
function AlifNetworkLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('AlifNetwork | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			AlifNetworkLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			AlifNetworkLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "rayfieldOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(AlifNetworkFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('AlifNetwork Settings', 0, true)

	if TabList['AlifNetwork Settings'] then
		TabList['AlifNetwork Settings'].LayoutOrder = 1000
	end

	if Elements['AlifNetwork Settings'] then
		Elements['AlifNetwork Settings'].LayoutOrder = 1000
	end
...
function AlifNetworkLibrary:CreateWindow(Settings)
	print('creating window')
	if AlifNetwork:FindFirstChild('Loading') then
		if getgenv and not getgenv().rayfieldCached then
			AlifNetwork.Enabled = true
			AlifNetwork.Loading.Visible = true

			task.wait(1.4)
			AlifNetwork.Loading.Visible = false
		end
	end
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				AlifNetworkLibrary:Notify({Title = 'Build Mismatch', Content = 'AlifNetwork may encounter issues as you are running an incompatible interface version ('.. ((AlifNetwork:FindFirstChild('Build') and AlifNetwork.Build.Value) or 'No Build') ..').\n\nThis version of AlifNetwork is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if isfolder and not isfolder(AlifNetworkFolder) then
		makefolder(AlifNetworkFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "AlifNetwork"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "AlifNetwork Interface Suite" then
		LoadingFrame.Version.Text = "AlifNetwork UI"
	end
	...
	if not Settings.DisableAlifNetworkPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				AlifNetworkLibrary:Notify({
					Title = "AlifNetwork Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(AlifNetworkFolder.."/Discord Invites") then
			makefolder(AlifNetworkFolder.."/Discord Invites")
		end

		if isfile and not isfile(AlifNetworkFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(AlifNetworkFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"AlifNetwork RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(AlifNetworkFolder.."/Key System") then
			makefolder(AlifNetworkFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("AlifNetwork | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AlifNetwork specific development.')
				end
			end
		end
		...
		if isfile and isfile(AlifNetworkFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(AlifNetworkFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(AlifNetworkFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						AlifNetworkLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called AlifNetworkLibrary:Destroy()
				...
				if not Success then
					...
					print("AlifNetwork | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AlifNetwork specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= AlifNetworkLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AlifNetworkLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			AlifNetwork.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("AlifNetwork | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AlifNetwork specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					AlifNetworkLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("AlifNetwork | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AlifNetwork specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					AlifNetworkLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("AlifNetwork | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AlifNetwork specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AlifNetworkLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= AlifNetworkLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AlifNetwork | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AlifNetwork specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						AlifNetworkLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("AlifNetwork | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with AlifNetwork specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AlifNetworkLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			AlifNetworkLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			AlifNetworkLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('AlifNetwork had an issue creating settings.') end
...
function AlifNetworkLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function AlifNetworkLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function AlifNetworkLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['AlifNetwork Settings'])
		end)
	end)
...
function AlifNetworkLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				AlifNetworkLibrary:Notify({Title = "AlifNetwork Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			AlifNetworkLibrary:Notify({Title = "AlifNetwork Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('AlifNetwork Configurations Error | '..tostring(result))
			AlifNetworkLibrary:Notify({Title = "AlifNetwork Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = AlifNetworkLibrary:CreateWindow({
	--	Name = "AlifNetwork Example Window",
	--	LoadingTitle = "AlifNetwork Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using AlifNetwork may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like AlifNetwork to get the key from
	...
	----AlifNetworkLibrary:Notify({Title = "AlifNetwork Interface", Content = "Welcome to AlifNetwork. These - are the brand new notification design for AlifNetwork, with custom sizing and AlifNetwork calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(AlifNetworkLibrary.Theme) do
...
task.delay(4, function()
	AlifNetworkLibrary.LoadConfiguration()
